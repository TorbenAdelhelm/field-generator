...
from typing import Tuple, Optional, Literal, Type, List
import h5py
import numpy as np
...
def fit_variogram_ensemble(
    ...
    *,
    ...
    for i in range(n):
        K = K_ens[i]
        field = np.log10(K, where=K > 0) if data_scale.lower() == "log10" else K

        # Empirical (isotropic) variogram on structured grid with fixed bins
        # Note: use vario_estimate_structured to honor structured coordinates.
        # try:
        #     bin_center, gamma = gs.vario_estimate_structured(
        #         pos_structured, field, bin_edges=bin_edges
        #     )
        # except AttributeError:
            # Fallback if gstools version lacks *_structured helper
            # (generic estimator accepts (X,Y) 2D arrays as well)
        #Xg, Yg = np.meshgrid(x, y, indexing="xy")
        bin_center, gamma = gs.vario_estimate(
            (x, y), field, bin_edges=bin_edges
        )

        gamma_stack.append(gamma)

        # Fit the chosen model class
        fit_model = model_cls(dim=2)
...
def plot_ensemble_variogram_summary(
    ...
    kwargs = {"dim": 2, "var": var_med, "len_scale": len_med}
...
def main():
    ...
    print(df.head())  # fitted parameters per realization
